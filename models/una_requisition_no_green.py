...
class UnaRequisition(models.Model):
    ...
    def _check_stock_availability(self):
        ...
        # ✅ NO CONVERSION - use quantity as-is (user enters in base units)
    ...
    #
    #             # ✅ FIND INVENTORY FOR CONSUMABLES
    #             if req.product_id.product_type == 'consumable':
    #                 # 🏢 Get the Admin Office (central store)
    #                 admin_office = self._get_admin_office_location()
    #
    #                 # 1️⃣ FIRST: Check Admin Office (central store)
    #                 inventory = self.env['una.inventory'].search([
    ...
    #                         )
    #
    #                 # 2️⃣ SECOND: Check destination location (if different from Admin Office)
    ...
    #                             )
    #
    #                 # 3️⃣ THIRD: Check ANY location with stock (fallback)
    ...
    #                             )
    #
    #                 # ❌ Error if no inventory found anywhere
    #                 if not source_location:
    ...
    #                         )
    #
    #                 # ✅ RESERVATION: Reserve stock (NO VALIDATION - already checked at submission)
    #                 if inventory_record and req.product_id.product_type == 'consumable':
    #                     # Reserve the stock (no need to check availability again)
    #                     inventory_record.reserved_quantity += req.quantity
    #
    #                     if req.product_id.is_bundle and req.product_id.bundle_quantity > 0:
    #                         reserved_bundles = req.quantity / req.product_id.bundle_quantity
    #                         req.message_post(
    #                             body=f"🔒 Reserved {req.quantity} {req.product_id.uom_id.name} ({reserved_bundles:.1f} {req.product_id.bundle_uom_id.name}) from {inventory_record.location_id.name} "
    #                                  f"(On Hand: {inventory_record.quantity_on_hand}, "
    #                                  f"Reserved: {inventory_record.reserved_quantity}, "
    #                                  f"Available: {inventory_record.available_quantity})"
    #                         )
    #                     else:
    #                         req.message_post(
    #                             body=f"🔒 Reserved {req.quantity} {req.product_id.name} from {inventory_record.location_id.name} "
    #                                  f"(On Hand: {inventory_record.quantity_on_hand}, "
    #                                  f"Reserved: {inventory_record.reserved_quantity}, "
    #                                  f"Available: {inventory_record.available_quantity})"
    #                         )
    ...
    #
    #             # ✅ NO BUNDLE CONVERSION - User enters quantity in base units
    ...
    #             }
    #
    #             # ✅ SET SOURCE LOCATION FOR CONSUMABLES
    #             if source_location:
    #                 move_vals['source_location'] = source_location
    #
    #             stock_move = self.env['una.stock.move'].create(move_vals)
    #
    #             # ✅ FORCE SET SOURCE LOCATION (in case it got lost during creation)
    ...
    #                 )
    #
    #             # ✅ NOW confirm (source_location is definitely set)
    #             if req.product_id.product_type == 'consumable':
        ...
        for req in self:
            ...
            if req.product_id.product_type != 'asset' or not req.product_id.is_serial_tracked:
                ...
                if req.product_id.product_type == 'consumable':
                    ...
                    # ❌❌❌ REMOVE THIS ENTIRE BLOCK - DO NOT RESERVE HERE ❌❌❌
                    # The reservation will happen when the stock move is confirmed
                    # if inventory_record and req.product_id.product_type == 'consumable':
                    #     inventory_record.reserved_quantity += req.quantity
                    #     ... message posts ...
    ...
    def _get_admin_office_location(self):
        """
        Get the system-wide Admin Office location.
        Uses the is_admin_office boolean field.
        """
        # 1️⃣ Find location marked as Admin Office
        ...
        # 2️⃣ Fallback: Find location with "ADMIN" in name
        admin_office = self.env['una.location'].search([
            ('name', 'ilike', 'ADMIN'),
            ('active', '=', True)
        ], limit=1)
        ...
        # 3️⃣ Last resort: Return the first active location
